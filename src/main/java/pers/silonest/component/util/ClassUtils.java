...
public abstract class ClassUtils {
   ...
   * {@link org.springframework.aop.support.AopUtils#getMostSpecificMethod}, this method does
   * <i>not</i> resolve Java 5 bridge methods automatically. Call
   * {@link org.springframework.core.BridgeMethodResolver#findBridgedMethod} if bridge method
   * resolution is desirable (e.g. for obtaining metadata from the original method definition).
   * <p>
   * <b>NOTE:</b> Since Spring 3.1.1, if Java security settings disallow reflective access (e.g.
   ...
   * implementations of the {@code GroovyObject} methods will be marked as synthetic anyway). Note
   * that, despite being synthetic, bridge methods ({@link Method#isBridge()}) are considered as
   * user-level methods since they are eventually pointing to a user-declared generic method.
   * 
   ...
   * in the same package as a class file, although
   ...
   * fed directly to {@code ClassLoader.getResource()}. For it to be fed to
   * {@code Class.getResource} instead, a leading slash would also have to be prepended to the
   * returned value.
   * 
   ...
   * <p>
   * Basically like {@code AbstractCollection.toString()}, but stripping the "class "/"interface "
   ...
   *         {@code null} if none found. If any of the given classes is {@code null}, the other
